...
function matchSeq(nodes, index, input, pos) {
  ...
  if (node.type === "char") {
    if (input[pos] === node.value)
      return matchSeq(nodes, index + 1, input, pos + 1);
    return -1;
  }
...
function matchNode(node, input, pos) {
  ...
  if (node.type === "concat") return matchSeq(node.children, 0, input, pos);
  if (node.type === "or") return matchSeq([node], 0, input, pos);
  return -1;
}
